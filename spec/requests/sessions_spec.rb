...
describe 'Session' do
    ...
    describe 'Session#create - ログイン' do
        ...
        context 'パスワードが違う場合' do
            example 'リクエストが失敗すること' do
                graphicker
                post '/login', params: FactoryBot.attributes_for(:graphicker, :invalid_password)
        ...
        context '存在しないユーザを指定した場合' do
            example 'リクエストが失敗すること' do
                graphicker
                post '/login', params: FactoryBot.attributes_for(:graphicker, :invalid_graphicker)
                expect(response.status).to eq(422)
            end
    ...
    describe 'Session#delete - ログアウト' do
        ...
        context 'ログインしていない場合' do
            example 'リクエストが失敗すること' do
                graphicker
                post '/logout', params: FactoryBot.attributes_for(:graphicker)
                expect(response.status).to eq(422)
            end
        end
